...
- 2026-06-20: **BR2 — historical payments linked to their invoices** (`payments.financial_document_id` backfilled on prod for the 25 received payments matching an outbound customer invoice 1:1; scaduto `FPA 1/23` excluded). Data-only one-shot script (`scripts/br2-link-payments-financial-documents.sql`), no view/dashboard/fiscal/AI value change (verified: 0 views read the FK, fiscal estimate reads only amount/payment_date/status). **Data-fattura (competence) attribution is STILL inert**: the link is now present but no code joins payment→document to derive an `issue_date`-based revenue attribution — that remains a dedicated fiscal spec (cassa→competenza). Known residual: a future re-import of one of the 25 historical XMLs would settle-in-place and overwrite `payment_date` with the doc `issue_date` (cross-year cassa shift), scoped out of BR2.
