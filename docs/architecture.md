...
- 2026-06-22: Profili di fatturazione cliente — introdotto il contratto backend
  `client_billing_profiles` per gestire intestatari fiscali diversi dal cliente
  operativo senza duplicare clienti, progetti o contatti. `clients` resta
  l'account operativo; `financial_documents.billing_profile_id` collega
  opzionalmente una fattura al profilo fiscale usato; la view
  `financial_documents_summary` espone i campi `billing_profile_*` mantenendo
  supplier, timezone Europe/Rome e `security_invoker`. Caso guida: Gustare
  Sicilia operativo con LIVE SRLS come intestatario fattura. La gestione
  ...
  fonte primaria per navigazione, filtri, export e incassi. L'import fatture
...
- 2026-06-22: Crediti inesigibili / write-off operativo — `payments.status`
  include `perso` con `writeoff_date` e `writeoff_reason` obbligatori. Le view
  `project_financials` e `client_commercial_position` espongono
  `total_written_off` e sottraggono il write-off solo da `balance_due`, mai da
  cassa/fiscalita'. Dashboard, scadenzario, badge scaduti, Payment UI, quote
  summary e AI pending usano stati aperti espliciti (`in_attesa|scaduto`).
  `invoice_void` rifiuta `perso`; `invoice_import_confirm` salta un re-import
  collegato a pagamento app-emesso gia' `perso`. Deploy remoto: migration DB +
  deploy Supabase della Edge Function `invoice_import_confirm`.
...
- 2026-06-20: Saldo scadenzario usa gli ACCONTI REALI (dichiarazione anno-2 chiusa), non la stima. La card "Scadenze fiscali" mostrava `7.941,49 €` sottostimando: il saldo sottraeva acconti STIMATI (formula su anno-2) invece dei reali versati. Helper puro `resolvePriorAdvanceScheduleInput(estimateInput, priorBasisDeclaration)`: se la dichiarazione (`currentYear-2`) è CHIUSA (`isDeclarationClosed`), deriva gli acconti dalla sua competenza (`definitiveInpsCompetenza` = `total_inps − prior_advances_inps`; `definitiveImposta` = `total_substitute_tax`), altrimenti fallback alla stima (byte-for-byte invariato). `buildFiscalModel` lo usa per il `priorAdvancePlan`; `buildFiscalPaymentSchedule`/`buildAdvancePlanFromEstimate` INTOCCATI → `fiscalParity.test.ts` verde. Caller `useDashboardData` fetcha `["fiscal-declaration", year-2]` e lo passa via `buildDashboardModel` → `buildFiscalModel`; copre desktop `DashboardAnnual` + mobile `MobileDashboard` (stesso hook, UI-7). Prod 2026: saldo INPS `2.839 → 3.571`, totale `~7.941 → ~8.840` (acconti reali 2025: 1.503,09 INPS + 233 imposta = 80%/100% della dichiarazione 2024 reale). DOM-8: `total_inps` solo letto, mai scritto. Clamp `≥0` già presente. INERTE per anni con obblighi certificati (il reale prevale in `buildFiscalRealityAwareSchedule`). Controllori: `resolvePriorAdvanceScheduleInput.test.ts` (3) + integrazione in `fiscalModel.test.ts` (`saldoWithout − saldoWith ≈ 1503,2`, falsificabile). EF `_shared/fiscalDeadlineCalculation` (reminder) resta sulla stima → follow-up DOM-5 due-layer. Review RAG :8001 multi-lente (fiscale/parità/TDD) + sorgente, PASS.

- 2026-06-20: Guardrail "obblighi certificati" + pulizia righe-spazzatura fiscali (branch `main`). Bug prod: la card "Scadenze fiscali" mostrava un falso `11.100,60 €` "Da dichiarazione" perché 6 `fiscal_obligations` `payment_year=2026` — proiezioni hand-inserite il 2026-04-14 col metodo "aliquota effettiva" già rigettato (DOM-8), con `declaration_id` NULL o legato alla dichiarazione 2025 a totali ZERO, 0 F24 — venivano trattate come reali. Tre superfici (sweep RAG :8001 + verifica sorgente) usavano la sola presenza della riga come "dato reale": `buildFiscalRealityAwareSchedule` (card), EF `fiscal_deadline_check.applyRealObligations` (promemoria/WhatsApp), `useDashboardData:102` (switch deduzione-cassa). Fix: (a) DELETE deterministico delle 6 righe su prod (0 F24 collegati, `RETURNING`, backup `*_backup_20260414`); (b) helper PURO `selectCertifiedObligations` (client `dashboard/` + mirror Deno `_shared/`): un obbligo è CERTIFICATO sse ha F24 OPPURE `declaration_id` → dichiarazione DEPOSITATA (`total_substitute_tax + total_inps > 0`); `source` NON discrimina (i reali sono `manual` anch'essi). Applicato in `useFiscalReality` (copre card desktop `DashboardAnnual` E mobile `MobileDashboard` in un punto, UI-7) e in `applyRealObligations` (EF, deploy separato). Provider: nuovo `getFiscalDeclarations()`. Discriminatore coerente con `buildObligationsFromDeclaration` (emette obblighi solo per `amount>0` linkati a dichiarazione reale). Controllore falsificabile `selectCertifiedObligations.test.ts` (6 test, riproduce il garbage 2026 → resta solo il bollo F24). Follow-up noto (NON fatto): `useDashboardData` usa ancora `obligations.length===0` come switch deduzione-cassa (dovrebbe gatare su dichiarazione depositata dell'anno; cambia la stima 2025 → test dedicato, vedi backlog).

- 2026-06-19: Colonna "Da saldare" nella lista clienti (#19, branch `feat/client-list-da-saldare`). La lista clienti non mostrava il residuo per-cliente (bisognava aprire ogni `ClientShow`). Aggiunta colonna desktop (ultima, allineata a destra, `tabular-nums`) + riga valore in `ClientMobileCard` (UI-7), sorgente = vista canonica `client_commercial_position.balance_due` (SYSTEM-FIRST, stessa fonte di `ClientFinancialSummary`/QW2, nessun ricalcolo). `ClientListContent` legge la vista via `useGetList` (perPage 1000, stessi params di `useDashboardData` → cache RQ condivisa), Map per `String(client_id)`. Helper PURO `clientBalanceCell.ts` (`formatClientBalanceCell`: `>0` "Da saldare" rosso, `<0` "Credito cliente" blu, `=0` "—" muted; segno-aware, `Math.abs`). Colonna in `CLIENT_COLUMNS` (`exportKey:"da_saldare"`) → integra column-visibility + resizable + export; l'exporter CSV fetcha la vista (full-view, NO `@in`) e include `da_saldare` solo quando la colonna è visibile (`filterExportRow`). `ClientFinancialSummary` NON refactorata: la sua presentazione dello zero diverge per design (`€ 0,00` in MetricCard vs "—"); la verità condivisa è il numero dalla stessa vista, non il formatter. Sort/filter per `balance_due` fuori scope (campo di vista, serve provider custom). Caveat noto (R5): la colonna parte visibile solo per chi non ha pref colonne `clients` salvata; se nascosta, toggle una volta da `ColumnVisibilityButton`. Controllori: unit `clientBalanceCell.test.ts` (falsificabile sign→label→color→abs) + `clientExportBalance.test.ts` (export field-survival: visibile→incluso, nascosto→assente) + e2e `client-list-balance.smoke.spec.ts` (valore seed reale `2984,50`). Frontend-only: zero DB/EF/migration/provider. 2 review spec + 2 review impl (frontend/mobile + data/provider/TDD), tutte code RAG :8001 + verifica sorgente, PASS. WF-17 browser desktop+mobile: 0 errori console. impeccable (product): colonna dedicata scannabile, non badge inline. typecheck/lint/build verdi, 657 unit, e2e verde. Spec/piano: `docs/superpowers/specs|plans/2026-06-19-client-list-da-saldare-column*`.

- 2026-06-19: Parità mobile scadenzario + cassa (QW3, branch `feat/qw3-mobile-scadenzario-cassa`). `MobileAnnualDashboard` non rendeva due card già presenti su desktop per l'anno corrente: `DashboardCashFlowCard` (previsione cassa 30gg) e `DashboardDeadlineTracker` (scadenzario "Cosa devi fare"). I dati erano già lato mobile (`useDashboardData` → `cashFlowForecast`/`alerts`): mancava solo il wiring (UI-7). Aggiunte in `MobileDashboard.tsx` dopo `DashboardKpiCards` e prima del blocco fiscale, gating identico al desktop (`isCurrentYear && data.cashFlowForecast` per la cassa, `isCurrentYear` per lo scadenzario; `cashFlowForecast` è già `null` per gli anni passati). Riuso verbatim dei componenti esistenti (già mobile-first): nessun nuovo componente, nessun prop `compact`, niente provider/EF/migration (smallest correct layer, view-only). Controllori: component test falsificabile `MobileDashboard.parity.test.tsx` (gating current/past year, date-independent, mutation-proven RED→GREEN) + smoke e2e `tests/e2e/mobile-dashboard-parity.smoke.spec.ts` (viewport 390px PRIMA del login, assert mobile shell + entrambe le card + riga "Scaduti"/"Incassato"). Audit impeccable `adapt`: port di parità, non redesign; note browser-only (touch target riga scaduto `Button size=sm` h-8=32px < 44px ideale ma app-wide e tappabile; nome cliente truncato sotto pressione larghezza → degrada a truncation, non overflow, identico al componente desktop). 2 review spec + 2 review impl (frontend/mobile-parity + TDD/controllori), tutte con code RAG :8001 + verifica sorgente, PASS. WF-17 browser desktop+mobile: 0 errori console, layout pulito a 390px. 650 unit verdi, e2e mobile verde. Spec/piano: `docs/superpowers/specs|plans/2026-06-19-qw3-mobile-scadenzario-cassa*`.

- 2026-06-19: Avviso anti-doppione su `/payments/create` (FIX-3 gemello, scope A, branch `fix/payment-create-orphan-hint`). FIX-3 riconcilia l'incasso atteso emesso SOLO in `QuickPaymentDialog`; registrando un incasso dal form generico `/payments/create` l'atteso emesso resta orfano (stesso doppio conteggio in `pendingPaymentsTotal`/AI). Scope A (deciso dall'utente, esposizione prod 0 oggi): card di AVVISO non bloccante `ExpectedPaymentOrphanHint` in `src/components/atomic-crm/payments/PaymentInputs.tsx` — display-only, **create-only** (`useRecordContext()===undefined`, F1: il form è condiviso con `PaymentEdit`), che compare quando registrare qui orfanerebbe un atteso emesso e indirizza all'Incasso rapido del progetto (`<Link>` a `buildProjectShowPath(projectId)` → `/projects/<id>/show` pulito, niente banner AI/auto-open). Predicato puro `wouldOrphanExpectedPayment` in `projects/quickPaymentReconciliation.ts` = `decideQuickPaymentTarget(...).action !== "create"` (SYSTEM-FIRST, nessun gate riscritto). `buildProjectShowPath` esportato con `defaults={}` opzionale. Frontend-only, niente scrittura form, niente EF, niente migration. Client/quote create-payment sono solo handoff a `/payments/create` → ereditano il fix; `invoice_import_confirm` ha la sua riconciliazione (fuori scope). Scope C (settle reale nel form) rimandato a quando l'emissione da app è in uso. Controllori: unit `wouldOrphanExpectedPayment` + RTL `PaymentInputs.test.tsx` (on-load ricevuto, edit-hidden, gate flips, no-setValue, href). 3 review spec + 5 review piano/trasversali + 3 review impl (mutation-tested), code RAG :8001 + prose RAG :8002. WF-17 browser desktop+mobile con dati demo + cleanup (WF-19, 0 leftover). Spec/piano: `docs/superpowers/specs|plans/2026-06-19-payment-create-reconciliation*`.

- 2026-06-19: AI capability registry — descrizione `quick_payment` allineata al comportamento reale (IMPORTANT-5, branch `fix/ai-quickpayment-capability-desc`). Dopo FIX-3 l'incasso rapido SALDA l'incasso atteso collegato alla fattura emessa (riconciliazione) invece di creare sempre; le description di `quick_payment_dialog` e `project_quick_payment` in `src/lib/semantics/crmCapabilityRegistry.ts` dicevano solo "crea un pagamento" (incompleto, non falso). Aggiornate per descrivere settle/picker/create. Solo metadata di capability (testo letto dall'AISnapshot/EF come contesto): nessun cambiamento di comportamento runtime, nessun deploy EF.

- 2026-06-19: Card "Da incassare" reale (QW2, branch `fix/da-incassare-card`). La card KPI mostrava `pendingPaymentsTotal` (solo le righe `payments` non-`ricevuto` dell'anno: su prod 375, nascondeva il credito reale ~7k). Ora mostra il residuo REALE cumulativo Σ max(0, `balance_due`) dalla vista canonica `client_commercial_position` (cassa-aware: `total_paid` filtra `ricevuto`, unisce rami project + no-project — `20260401094930`). Funzione pura `src/lib/analytics/outstandingReceivables.ts` (`sumOutstandingReceivables` clamp per-cliente + `countOpenReceivables` + `formatOpenClientsSubtitle`). Fetch year-INDEPENDENT in `useDashboardData` (separato dal model year-scoped, I4), nuovo prop REQUIRED `outstandingReceivables:{total,count}` a `DashboardKpiCards` propagato da `DashboardAnnual` E `MobileDashboard` (UI-7, parità enforced da tsc). Card pulita Approccio Bambino: un numero + "N clienti con saldo aperto", barra rimossa. Coerenza AI (B2, no seconda verità): `buildAnnualOperationsContext` accetta `{outstandingReceivablesTotal}` opzionale → nuovo metric `outstanding_receivables_total` (basis `receivable`, cumulativo) + caveat; `pending_payments_total` MANTIENE l'id (la EF `_shared/annualOperationsAiGuidance.ts` legge per id il `.value`) con label disambiguata ("anno"). `dataProviderAnalyticsContext` fetcha la vista (stessa fonte della card) e passa il totale → frontend-only, NIENTE deploy EF, niente migration. Controllori: pura `outstandingReceivables.test.ts`, builder `buildAnnualOperationsContext.test.ts` (metric + relabel + omit), controllore B2 falsificabile `dataProviderAnalyticsContext.test.ts` (mock provider: metric = Σ balance_due ≠ pendingPaymentsTotal), e2e `dashboard-annual.smoke.spec.ts` (valore deterministico del fixture 2985 + "1 cliente con saldo aperto"). 3 review multi-superficie (code RAG :8001 + docs diretti) PASS. Grounding prod read-only: Σ = 7.131,37 su 3 clienti (era 6.697 quando scritta la spec — dato vivo evoluto). Spec/piano: `docs/superpowers/specs/2026-06-17-da-incassare-card-design.md`, `docs/superpowers/plans/2026-06-17-da-incassare-card.md`.

- 2026-06-19: Riconciliazione incasso atteso (FIX-3+4, branch `fix/expected-payment-reconciliation`). Chiude il doppio conteggio in `pendingPaymentsTotal` ("Da incassare") dovuto all'incasso ATTESO (`payments` `in_attesa` + `financial_document_id`) creato da `invoice_emit`. Due decider PURI sul pattern di `decideEmittedPaymentReconciliation` (match per `financial_document_id`, SYSTEM-FIRST): `src/components/atomic-crm/projects/quickPaymentReconciliation.ts#decideQuickPaymentTarget` (`settle|create|ambiguous`, gate `payment_type` B1: solo `saldo/acconto/parziale`) e `supabase/functions/_shared/invoiceEmit.ts#decideEmitExpectedPayment` (`absorb|create|ambiguous`, PROJECT-LEVEL ONLY B2). FIX-3: `QuickPaymentDialog` carica i candidati (`useGetList` `financial_document_id@not.is`=null, `enabled: open`), su match singolo fa `useUpdate` della riga (`status='ricevuto'`, `payment_date = paymentDate || todayISODate()` mai null/futuro — VP2/DOM-1) invece di creare, su >1 mostra un picker "quale fattura". FIX-4: `invoice_emit/index.ts` dentro la tx fa `SELECT ... FOR UPDATE` dei candidati manuali (FK NULL, stesso client+project), e su match singolo `UPDATE ... SET financial_document_id, invoice_ref WHERE financial_document_id IS NULL` + count-guard fail-closed (M1, 409) invece di un secondo insert; provider `dataProviderInvoiceEmit.EmitInvoiceResult` espone `expectedPaymentAbsorbed?: boolean` (I2). Niente migration (la FK esiste da `20260616200000`). Limite v1 documentato: un atteso manuale assorbito viene cancellato da `invoice_void` (nessuna cassa persa). Money-TDD: decider puri + RTL (`update` non `create`, cassa-year a clock congelato WF-9) + e2e contro EF reali (`tests/e2e/invoice-void.smoke.spec.ts`). 4 review multi-superficie + RAG (PASS). Spec/piano autoritativi: `docs/superpowers/specs/2026-06-17-expected-payment-reconciliation-design.md`, `docs/superpowers/plans/2026-06-17-expected-payment-reconciliation.md`. PROD: deploy EF `invoice_emit` (BE-1/BE-8) gated OK utente; FIX-3 frontend (Vercel al merge).

- 2026-06-17: Registro lavori friction quick-wins (branch `work/registro-lavori-friction`). `services/serviceBilling.ts` (helper puri `isServiceBilled`/`getServiceBillingState` con `className`/`buildProjectInvoiceEmitPath`). `ServiceShow`: ponte "Emetti la fattura del progetto" (gate `project_id && !isServiceBilled` → `/projects/<id>/show?invoiceDraft=true`) + badge fatturato. Badge "Fatturato/Da fatturare" in lista (`ServiceListContent`, inline) + card mobile (`ServiceMobileCard`) + filtro "Stato fatturazione" → "Da fatturare" (`invoice_ref@is:null`, pattern taskFilters) in `ServiceListFilter`. `ServiceInputs` `invoice_ref` `parse=(v)=>v||null` (mai `''`, filtro coerente). `ProjectShow` apre la bozza su `?invoiceDraft=true` anche senza importo fatturabile (mostra `InvoiceDraftEmptyState`, niente vicolo cieco dal ponte). Frontend-only, read-only + navigazione, nessuna migration/EF. Diagnosi UX `whah0a0sf` + 3 review multi-superficie (RAG, FLAG chiusa).

- 2026-06-16: Emetti fattura (invoice_emit) — FOUNDATION in corso (branch `work/invoice-emit`, NON in prod). Additive migration `20260616200000_payments_financial_document_link.sql`: `payments.financial_document_id` (FK `ON DELETE SET NULL`, indice) come ancora anti-doppione tra fattura emessa dall'app e re-import dell'XML. Kysely types estesi in `supabase/functions/_shared/db.ts` (`financial_documents`, `services`, `payments.financial_document_id`, `expenses.source_service_id`). Helper puri in `invoicing/`: `computeInvoiceDraftAmounts` (LORDO documento `taxable_amount`/`total_amount` vs NETTO da incassare `payment.amount`; distinto da `computeInvoiceDraftTotals` usato dall'XML, lasciato invariato) e `isInvoiceBillingComplete` (gate dei campi FatturaPA obbligatori issuer/cliente, DOM-3). I builder `buildInvoiceDraftFrom{Project,Client}` ora espongono `serviceIds[]`/`expenseIds[]` (esclusi gli expense con `source_service_id`, DB-8) per marcare i sorgenti come fatturati senza doppio conteggio km. Spec/piano autoritativi: `docs/superpowers/specs/2026-06-16-invoice-emit-design.md`, `docs/superpowers/plans/2026-06-16-invoice-emit.md` (3 review multi-superficie + RAG, BLOCK chiuse). Scope v1: niente acconto pregresso, niente quote/singolo service, niente storno. Seguono nello stesso branch: EF transazionale `invoice_emit` (pre-flight SELECT idempotente, payment `in_attesa` cassa-neutro), provider `emitInvoice`, ramo update-in-place in `invoice_import_confirm`, azione UI nel dialog bozza con branch mobile Sheet, badge stato incasso in `FinancialDocumentShow` (da payment via `financial_document_id`), e registry AI aggiornati (`crmCapabilityRegistry.generate_invoice_draft` + `crmSemanticRegistry` import reconciliation).

- 2026-06-19: D3 — anno con dichiarazione reale chiusa mostra il DEFINITIVO (no doppia verita'). Nuovo helper puro `src/components/atomic-crm/dashboard/applyDefinitiveDeclaration.ts`: per un anno chiuso (`total_substitute_tax + total_inps > 0`) sostituisce la stima della formula coi numeri reali del commercialista — INPS competenza = `total_inps − prior_advances_inps` (stessa derivazione di `buildObligationsFromDeclaration.inps_saldo`; `total_inps` ciclo riconciliato NON toccato, DOM-8), imposta = `total_substitute_tax`. Verificato su dichiarazioni AdE reali (prod read-only): 2023→INPS 2249/imposta 429, 2024→1879/233; 2025 ha totali zero → resta stima. Nuovo flag `FiscalKpis.isDefinitive` (`fiscalModelTypes.ts`); `buildFiscalModel`/`buildDashboardModel` accettano `declaration?` e applicano l'override su `estimate.fiscalKpis`; `useDashboardData` fetcha `getFiscalDeclaration(anno)` (queryKey `["fiscal-declaration", year]`, dedup react-query). UI: `DashboardFiscalKpis` + `MobileFiscalKpis` mostrano pill `Definitivo`/`Stima` (INV-6, parita' UI-7). Solo client+UI: la formula condivisa `buildFiscalYearEstimate` (client/EF) e' INTATTA; l'EF `_shared/fiscalDeadlineCalculation.ts` aggiunge solo `isDefinitive:false` per parita' di shape (`fiscalParity.test.ts` verde). Controllori: `applyDefinitiveDeclaration.test.ts` (oracoli), `DashboardFiscalKpis.test.tsx` (pill), E2E `fiscal-definitivo.smoke.spec.ts` (desktop+mobile reale, seed+cleanup WF-19) e `fiscal-estimate.smoke.spec.ts` (formula INPS stima). Browser-verificato desktop+mobile.
...
- 2026-04-14: Fiscal reality layer reconciliation hardening — migration `20260414192200_fiscal_interests_and_compensation.sql` extends `fiscal_obligations.component` with `interessi_erario` and `interessi_inps` so F24 rateazione interests (AdE `1668`, INPS `DPPI`) can be represented explicitly instead of being hidden inside principal rows. `fiscal_f24_submissions` now has `compensation_credit numeric(10,2) not null default 0` for real F24 deleghe that use credits in compensation; `fiscal_f24_payment_lines_enriched` exposes the field. Frontend sweep: `fiscalModelTypes.ts`, `fiscalRealityTypes.ts`, `buildFiscalRealityAwareSchedule.ts`, `ObligationEntryDialog.tsx`, `F24RegistrationDialog.tsx`, and `fiscalRealityProvider.ts` were aligned. `F24RegistrationDialog` now lets the user enter a compensation credit and shows `saldo delega = sum(righe) - compensation_credit`; checked line amounts are now validated as strictly `> 0`, matching the DB constraint on `fiscal_f24_payment_lines.amount`.
- 2026-04-14: DX — scoped Deno config for `supabase/functions/`. VSCode was treating the Edge Functions subtree as plain TypeScript, producing false errors on Deno-native imports (`jsr:...`, `npm:...`, `Deno.*`) and degrading autocomplete. Fix is non-functional (no runtime impact): `supabase/functions/deno.json` with Deno compilerOptions + `.vscode/settings.json` with `deno.enablePaths: ["supabase/functions"]` scoping Deno support only to that subtree. Recommended extension added via `.vscode/extensions.json`. `.gitignore` adjusted to allow the two `.vscode/*` files into the repo. Cherry-picked from abandoned branch `fix/unified-crm-answer-copy-and-km-audit` (commit `fbadf35d`); the sibling refactor `deebad06` was already on `main` as `863b48e4`, so after this commit the abandoned branch carries no unique value.
- 2026-04-14: Quick Episode description + timed events — `QuickEpisodeDialog` (the Puntata shortcut on `ProjectShow`) now exposes the same temporal fields as the full `ServiceCreate` form. New "Tutto il giorno" toggle that swaps the `service_date` / `service_end` inputs between `type="date"` and `type="datetime-local"`, and a new `description` input persisted on `services.description` (previously always NULL for Puntata-created rows). `buildQuickEpisodeServiceCreateData` in `src/components/atomic-crm/projects/quickEpisodePersistence.ts` no longer hardcodes `all_day: true` and forwards `description`, `service_date`, `service_end` through a new `toServicePersistenceDate` helper that keeps date-only strings untouched and normalizes datetime-local strings to ISO-with-offset via `new Date(...).toISOString()` — the one place where `new Date("YYYY-MM-DDTHH:mm")` is intentional, since `datetime-local` is explicitly browser-local semantic and not a business date-only string covered by rule WF-8. `buildQuickEpisodeExpenseCreateData` uses `toBusinessISODate` to keep `expenses.expense_date` as a `YYYY-MM-DD` date even when `service_date` carries a time component (the column is `date`, not `timestamptz`). Combined with the Google Calendar sync fix from commit `e7e8bded`, a timed Puntata now syncs to Google Calendar with the actual start/end hours.
- 2026-04-14: Google Calendar sync — fix hardcoded 09:00-18:00 for timed events. `buildCalendarEvent` in `supabase/functions/google_calendar_sync/index.ts` was a TODO left open: for non-all-day services it sliced only the date portion of `service_date` / `service_end` and concatenated a fixed `T09:00:00` / `T18:00:00`, discarding the actual start/end times the user had entered. Services stored at e.g. 08:30-14:30 Europe/Rome showed up on Google Calendar as 09:00-18:00. Fix: timed events now pass `service.service_date` and `service.service_end` verbatim as RFC3339 `dateTime` values (DB `timestamp with time zone` columns already carry the UTC offset; Google parses them directly). Fallback when `service_end` is null: start + 1h. EF redeployed to production remote (`qvdmzhyzpyaveniirsmo`).
- 2026-04-14: Quick Episode duplicate/orphan fix — `buildQuickEpisodeServiceCreateData` in `src/components/atomic-crm/projects/quickEpisodePersistence.ts` now inherits `client_id` from the selected project, so services created via the `QuickEpisodeDialog` are never orphaned from their client. `QuickEpisodeDialog.handleSubmit` now queries existing services on the same project/business-day (Europe/Rome window via `startOfBusinessDayISOString`/`endOfBusinessDayISOString`) and asks for explicit confirmation before creating a duplicate. New pure helpers `findExistingQuickEpisodeServices` and `buildQuickEpisodeDuplicateConfirmMessage` are unit-tested. Remote cleanup: migration `20260414162444_remove_quick_episode_duplicate_savoca_2026_04_11` removes the single pre-existing orphan (`acc079b0-...`) with a scoped DELETE; the linked km expense is cascaded via `expenses.source_service_id ON DELETE CASCADE`. The canonical service (`ed62a7bc-...`) is untouched. NB: the orphan's Google Calendar event (`5po1iql332gi8g3c8u8u3vtdtg`) is not removed by the migration and must be deleted manually from the calendar.
- 2026-04-13: Supabase linter security fixes — migration `fix_security_invoker_on_views` sets `security_invoker = on` on 3 views (`monthly_revenue`, `analytics_yearly_competence_revenue`, `financial_documents_summary`) that lost the flag after being recreated in later migrations. Migration `fix_function_search_path` pins `search_path = 'public'` on all 13 public functions to prevent search_path injection (Supabase linter 0010/0011). Both applied to remote.
- 2026-04-02: Fiscal weekend due-date shift — new shared helper `shiftWeekendToNextBusinessDay` added in client + Edge Function `dateTimezone` modules. Estimated fiscal deadlines (`fiscalDeadlines.ts`, `_shared/fiscalDeadlineCalculation.ts`) and declaration-derived obligations (`buildObligationsFromDeclaration.ts`) now move Saturday/Sunday due dates to the next Monday instead of keeping raw `30/11` / `31/05`. This aligns the system with real fiscal due dates such as `2024-12-02`, `2025-12-01`, and `2026-06-01`. Tests added on both runtimes.
...
- `financialDocuments` (documenti fiscali emessi/ricevuti, dato di
  fatturato/emissione non di cassa) + `financialDocumentsCaveat`
...
| fiscal_obligations | Singole rate/scadenze da pagare derivate dalla dichiarazione o create manualmente | auth.uid() = user_id | FK `declaration_id` (bare, no cascade), `source` (auto_generated/manual), `component` (enum 10 valori: imposta/inps + `bollo` + `interessi_erario` + `interessi_inps`), `competence_year`, `payment_year`, `due_date`, `amount`, `installment_number/total`, `is_overridden`, note, timestamps |
| fiscal_f24_submissions | Un evento di versamento F24 | auth.uid() = user_id | `submission_date`, `notes`, `compensation_credit >= 0`, timestamps |
...
Nota backup fiscali 2026-04-14:

- Le tabelle `fiscal_*_backup_20260414` sono snapshot manuali di recupero, non
  superfici runtime del prodotto.
- Devono restare non accessibili da `anon`, `authenticated` e `PUBLIC`.
- RLS e privilegi sono verificati da
  `npm run security:check:fiscal-backups`.
- REST anon e' verificato da
