...
- 2026-04-15: Fiscal "next deadline" card — user-facing tax estimate rebuilt on real-data baseline. New pure function `computeTaxSchedule` (in `dashboard/computeTaxSchedule.ts`) consumes `fiscal_declarations` + `fiscal_obligations` + `payments` + enriched F24 payment lines and returns `{baseline, ytd, upcomingDeadlines, totalCashOutYear, totalSaldoPortion, totalAnticipoPortion}`. The baseline is the most recent closed-year declaration with non-zero totals; the effective rate is `(total_sost + total_inps) / reported_revenue`, which for Rosario 2024 is 28.39% — much higher than the theoretical forfettario formula (78% × 26.07% × 5% ≈ 20.3%) because real fiscal totals include maternità, conguagli, minimali and rounding choices of the accountant. New `DashboardNextDeadlineCard` renders the view additively in `DashboardAnnual` and `MobileDashboard` above the existing theoretical `DashboardFiscalKpis` (old card kept as a secondary "simulation" view). New `listFiscalDeclarations()` method on `fiscalRealityProvider` returns all declarations ordered by year. The `DichiarazioneEntryDialog` in both desktops no longer receives the `selectedYear` fiscal KPIs as `estimatedSubstituteTax` / `estimatedInps` — they were the wrong year (e.g. 2026 YTD) and produced bogus "Diverge >30%" warnings when the user entered a real 2025 total. Rule `DOM-6` added to `.claude/rules/learning.md`.
- 2026-04-14: Fiscal reality layer reconciliation hardening — new migration `20260414211500_fiscal_interests_and_compensation.sql` extends `fiscal_obligations.component` with `interessi_erario` and `interessi_inps` so F24 rateazione interests (AdE `1668`, INPS `DPPI`) can be represented explicitly instead of being hidden inside principal rows. `fiscal_f24_submissions` now has `compensation_credit numeric(10,2) not null default 0` for real F24 deleghe that use credits in compensation; `fiscal_f24_payment_lines_enriched` exposes the field. Frontend sweep: `fiscalModelTypes.ts`, `fiscalRealityTypes.ts`, `buildFiscalRealityAwareSchedule.ts`, `ObligationEntryDialog.tsx`, `F24RegistrationDialog.tsx`, and `fiscalRealityProvider.ts` were aligned. `F24RegistrationDialog` now lets the user enter a compensation credit and shows `saldo delega = sum(righe) - compensation_credit`; checked line amounts are now validated as strictly `> 0`, matching the DB constraint on `fiscal_f24_payment_lines.amount`.
