...
This project uses Vitest for unit tests and Prettier for formatting checks.
Run the main quality checks with the following commands:
...
`make lint` runs ESLint first, then performs a repository-wide Prettier check.
...
- When you add a reusable component under `src/components/atomic-crm`, check whether the hook regenerated `registry.json` and include that change in your commit.
