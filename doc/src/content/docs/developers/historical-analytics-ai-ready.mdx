...
The current annual dashboard loads multiple raw resources client-side. That is acceptable for a small operational dashboard, but it is a weak base for historical analytics and future AI features.
...
For local demo compatibility, choose one of these strategies:

- add synthetic `analytics_*` resources in the FakeRest generator,
- or disable the historical dashboard in demo mode with a clear notice.

The preferred option is to generate synthetic `analytics_*` resources. This keeps the semantic contract identical between demo and production.
