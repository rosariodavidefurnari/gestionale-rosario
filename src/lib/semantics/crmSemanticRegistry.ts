  ...
  return {
    ...
    rules: {
      ...
      invoiceImport: {
        ...
        meaning:
          "L'import fatture nella chat AI unificata deve proporre record strutturati, trasportare anche l'anagrafica fiscale letta dal documento e poi mappare le fatture cliente su payments e le fatture/costi fornitore su expenses solo dopo conferma utente. La conferma reale deve passare da validazioni server-side, coerenza cliente/progetto, controllo duplicati e rollback completo del batch se un record non e' confermabile. Se l'XML re-importato corrisponde a una fattura GIA EMESSA dall'app (match per cliente + invoice_ref con ancora primaria payments.financial_document_id), invece di creare un nuovo payment l'import aggiorna in-place l'incasso atteso (in_attesa -> ricevuto) e collassa le N righe della fattura su quel singolo incasso (anti doppio conteggio); i payment in_attesa creati a mano non vengono toccati. Se manca il cliente, il passo corretto e' aprire il form cliente gia precompilato con quei campi, non creare automaticamente il record.",
      },
      unifiedAiReadContext: {
        scope:
