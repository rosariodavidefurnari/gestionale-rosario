...
import { historicalCashInflowSuggestedQuestions } from "@/lib/analytics/historicalCashInflowAnalysis";
...
export const DashboardHistoricalCashInflowAiCard = () => {
  ...
  const {
    ...
    mutationKey: ["historical-cash-inflow-ai-summary"],
    mutationFn: () => dataProvider.generateHistoricalCashInflowSummary(),
  ...
  const {
    ...
    mutationKey: ["historical-cash-inflow-ai-answer"],
    mutationFn: (nextQuestion: string) =>
      dataProvider.askHistoricalCashInflowQuestion(nextQuestion),
    onError: (mutationError: Error) => {
      notify(
        mutationError.message ||
      ...
      <CardHeader className="space-y-3">
        <div className="flex items-center justify-between gap-3">
          <CardTitle className="text-base flex items-center gap-2">
            ...
            AI: spiegami gli incassi
          </CardTitle>
          <Badge variant="outline">{selectedModel}</Badge>
        </div>
        <p className="text-xs text-muted-foreground">
          Qui l&apos;AI legge solo i soldi già ricevuti, non il valore del
          lavoro. Serve per capire come stanno entrando gli incassi nel tempo.
        </p>
      </CardHeader>
      ...
      <CardContent className="space-y-4">
        {summary ? (
          ...
          <div className="rounded-md border border-dashed px-4 py-6 text-sm text-muted-foreground">
            ...
            spiegare gli incassi storici in italiano semplice.
          </div>
        )}
        ...
        <div className="flex justify-end">
          ...
          >
            ...
            {summary ? "Rigenera spiegazione" : "Spiegami gli incassi"}
          </Button>
        </div>
        ...
        <div className="space-y-4">
          <div className="space-y-1">
            ...
            <p className="text-xs text-muted-foreground">
              Qui l&apos;AI usa solo gli incassi già ricevuti. Se un confronto
              non è corretto o non è dimostrabile, te lo dice.
            </p>
          </div>

          <div className="space-y-2">
            <Label htmlFor="historical-cash-inflow-ai-question">
              Fai una domanda sugli incassi
            </Label>
            <Textarea
              id="historical-cash-inflow-ai-question"
              value={question}
              onChange={(event) => setQuestion(event.target.value)}
              placeholder="Per esempio: il 2026 finora è avanti o indietro rispetto al 2025?"
              maxLength={300}
              className="min-h-24"
            />
          ...
          <div className="flex flex-wrap gap-2">
            {historicalCashInflowSuggestedQuestions.map((suggestion) => (
              <Button
                key={suggestion}
                type="button"
                variant="secondary"
