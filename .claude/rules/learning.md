...
**Update 2026-06-20 (gate 1 EF reminder)**: il LAYER STIMA della EF `fiscal_deadline_check` è ora allineato alla card "Scadenze fiscali" sul SALDO: `buildFiscalReminderComputation` accetta `basisContributiVersatiCassa` (imposta del saldo dedotta su CASSA LM035, anno-1) e `priorBasisDeclaration` (acconti del saldo dai numeri REALI della dichiarazione chiusa anno-2, DB-12). La EF `loadEstimateRealityInputs` fetcha dichiarazione `tax_year=anno-2` + obblighi/F24 `payment_year=anno-1` e passa i 2 input — stessa fetch del hook `useDashboardData` (parità). Helper puri Deno mirrorati (`resolvePriorAdvanceScheduleInput`, `isDeclarationClosed`, `definitive*` in `_shared/fiscalDeadlineCalculation.ts`; `sumInpsContributionsPaidInYear` in `_shared/inpsContributionsPaid.ts`), parità garantita da `fiscalParity.test.ts` (scenario `schedule_realPriorAdvances_plus_cashImposta`, falsificabile). Controllore prod read-only: `npm run smoke:ef-reminder-parity` (= 9.005,91 sul 2026). La EF resta inerte per anni con obblighi certificati (overlay `applyRealObligations` vince, DB-11). `total_inps` solo letto (DOM-8). Resta da allineare: la SOGLIA/notifica resta stima per anni senza dichiarazione chiusa (fallback corretto).
