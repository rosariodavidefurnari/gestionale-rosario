...
- usa \`resource = "services"\` SOLO se il documento e' un riepilogo lavori INTERNO e NON fiscale (es. CSV/scheda lavoro, notula informale) — MAI per una FATTURA/FatturaPA EMESSA dal titolare: quella va SEMPRE su \`resource = "payments"\` (vedi REGOLA CLASSIFICAZIONE sopra), anche se elenca spot/prestazioni in tabella. Una fattura emessa con tabella di righe = UN record \`payments\`, non N record \`services\`. REGOLA CRITICA (solo per i riepiloghi interni non fiscali): se il documento contiene una tabella con N righe di servizi/lavori, DEVI produrre N record separati nell'array records — uno per ogni riga della tabella. Non raggruppare, non sommare, non creare un singolo record riassuntivo. Ogni riga = un record. Per ciascun record:
